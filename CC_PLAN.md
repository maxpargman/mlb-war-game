...
- [x] **4.4 Add `device_id` to `daily_scores`.** Same column as `game_sessions`, populated by `submit-score` from the request body (device UUID already generated client-side in `session.ts`). Lets leaderboard rows be correlated back to a session/device.
